...
void GBK_ReadID(void)
{
	...
	FontLib_Read(Temp,0,47);
	
//	LCD_Clear(WHITE);//清除屏幕
	
//	for(i=0;i<24;i++)Temp[i]=0;//初始化缓冲区
//  FontLib_Read(Temp,0,17);  										// mcudev.taobao.com
//  DrawFont_GBK16B(16,16,RED,Temp);
	
//	for(i=0;i<24;i++)Temp[i]=0;//初始化缓冲区
//  FontLib_Read(Temp,20,8);   									// DevEbox
//	DrawFont_GBK16B(16,32,RED,Temp);
//	
//	for(i=0;i<24;i++)Temp[i]=0;//初始化缓冲区
//  FontLib_Read(Temp,30,8);   									// 大越电子
...
void GBK_LibFont_Test(void)
{
	...
	DrawFont_GBK16B(24,16,BLUE,"飞流直下三千尺");
	delay_ms(1000);
